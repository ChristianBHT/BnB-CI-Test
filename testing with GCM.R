...
clusterEvalQ(cl, c(library('caret'), library('ipred'), library('GeneralisedCovarianceMeasure')))
for (N in c(200,400,800,1600,3200)){
  clusterExport(cl,  varlist=c('uniform_noise', 'N'), envir=environment() )  
  res <- pblapply(cl=cl,1:100, function(i){
    set.seed(i)
    ...
    #Testing the wrong condition
    cond_var <- data.frame(data$X1)
    gcm_test_w <- gcm.test(data$X3, data$X4, Z = cond_var)
  ...
  results <- do.call(rbind, res)
  colnames(results) <- c('pvalue_right','test_S_right','pvalue_wrong', 'test_S_wrong')
  write.csv(as.data.frame(results),paste0("C:/ChristianThorjussen/results/GCM_uniform_noise_",N,".csv"), row.names = TRUE)
}
